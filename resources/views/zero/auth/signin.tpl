...
<html lang="vi">
	...
	<body id="kt_body" class="app-blank app-blank bgi-size-cover bgi-position-center bgi-no-repeat">
		...
		<div class="d-flex flex-column flex-root" id="kt_app_root">
			<style>body { background-image: url({if empty($config['website_auth_background_image'])}'/theme/infinity/media/auth/bg4.jpg'{else}{$config['website_auth_background_image']}{/if}); } [data-bs-theme="dark"] body { background-image: url({if empty($config['website_auth_background_image'])}'/theme/infinity/media/auth/bg4-dark.jpg'{else}{$config['website_auth_background_image']}{/if}); }</style>
