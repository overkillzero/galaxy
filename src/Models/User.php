...
class User extends Model
{
    ...
    public function disableReason(): string
    {
        ...
        if (is_null($reason)) {
            return 'Bị vô hiệu vì lý do đặc biệt, vui lòng liên hệ admin để biết chi tiết';
        }
        return $reason->text;
    }
    ...
    {
        return ($this->last_detect_ban_time == '1989-06-04 00:05:00' ? 'Không bị cấm' : $this->last_detect_ban_time);
    }
    ...
    public function productTrafficResetDate()
    {
        if (!is_null($this->reset_traffic_date)) {
            ...
            return I18n::get()->t('Không làm mới');
        }
    }
